...
public class TestGetReference {
	...
	public void shouldGetReferenceById(){
		String path = Nouns.REFERENCES + "/" + id;
	...
	public void shouldFailWithBadId(){
		String BadId = id + "a";
		String path = Nouns.REFERENCES + "/" + BadId;
		System.err.println("path=[" + path + "]pathogen_virus");
		Response response = target.path(path).request().accept(MediaType.APPLICATION_JSON).get();
	...
	public void shouldFailWithOutOfRangeId(){
		...
		String path = Main.BASE_URI + Nouns.REFERENCES + "/" + outOfRangeId;
	...
	@Test
	public void shouldGetReferenceOffsetAndLimit(){
	...
	public void shouldGetReferenceCount(){
		String path = Nouns.REFERENCES + "/" + Nouns.COUNTS + "/" + counter;
		System.out.println("path=[" + path + "]");
		Response response = target.path(path).request().accept(MediaType.APPLICATION_JSON).get();
		System.out.println(Response.Status.OK.getStatusCode() + " " + response.getStatus());
		assertEquals(Response.Status.OK.getStatusCode(), response.getStatus());
	...
	public void shouldFailByHostCountExceededTen(){
		int InvalidCount = counter + 2;
	 	String path =  Nouns.REFERENCES + "/" + Nouns.COUNTS + "/" + InvalidCount;
		System.out.println("path=[" + path + "]");
		Response response = target.path(path).request().accept(MediaType.APPLICATION_JSON).get();
		System.out.println(response);
		System.out.println(Response.Status.BAD_REQUEST.getStatusCode() + " " + response.getStatus());
