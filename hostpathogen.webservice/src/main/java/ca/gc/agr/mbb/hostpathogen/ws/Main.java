...
public class Main implements WSConstants {
	...
	static{
		try{
			BASE_URI = "http://" + getLocalHostLANAddress().getCanonicalHostName() + ":8080/" + BASEPATH;
	...
	public static LuceneConfig luceneConfig = null;
	...
	public HttpServer startServer() {
		...
		// in ca.gc.agr.mbb.hostpathogen.ws package
		final ResourceConfig rc = new ResourceConfig().packages("ca.gc.agr.mbb.hostpathogen.ws");
		rc.property(JsonGenerator.PRETTY_PRINTING, true);
		
		HPSearcher<Pathogen> hpsp = new HPSearcher(Pathogen.class);
		HPSearcher<Host> hpsh = new HPSearcher(Host.class);
		HPSearcher<HostPathogen> hpshp = new HPSearcher(HostPathogen.class);
		HPSearcher<Location> hpsl = new HPSearcher(Location.class);
		HPSearcher<Reference> hpsr = new HPSearcher(Reference.class);
		HPSearcher<Author> hpsa = new HPSearcher(Author.class);
